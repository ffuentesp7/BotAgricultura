...
def recomendacion(lista):
    aparicionRiego = 0
    aparicionTemperatura = 0
    aparicionHumedadDelSuelo = 0
    aparicionEvapotranspiracion = 0
    aparicionPrecipitaciones = 0
    aparicionVelocidadDelViento = 0
    for palabra in lista:
        if palabra in riego:
            aparicionRiego += 1
        if palabra in temperatura:
            aparicionTemperatura += 1
        if palabra in humedadDelSuelo:
            aparicionHumedadDelSuelo += 1
        if palabra in evapotranspiracion:
            aparicionEvapotranspiracion += 1
        if palabra in precipitaciones:
            aparicionPrecipitaciones += 1
        if palabra in velocidadDelViento:
            aparicionVelocidadDelViento += 1
    
    lista = []
    if(aparicionRiego>0):
        lista.append("riego")
    if(aparicionTemperatura>0):
        lista.append("temperatura")
    if(aparicionHumedadDelSuelo>0):
        lista.append("humedad")
    if(aparicionEvapotranspiracion>0):
        lista.append("evapotranspiracion")
    if(aparicionPrecipitaciones>0):
        lista.append("lluvia")
    if(aparicionVelocidadDelViento>0):
        lista.append("viento")
    return lista